...
#     if x == a[i]:
#         count += 1
# print("We found {} numbers {}".format(count,x))        
...
# for i in range(n):
#     if abs(a[i]-x) <= abs(foundFirst - x):
#         if foundFirst != a[i]:
#             foundSecond = foundFirst
#         foundFirst = a[i]    
# if abs(foundSecond - x) == 2 and abs(foundFirst - x) == 2:
#     print("We found the two numbers", foundFirst,foundSecond)
# else:
#     print("We found",foundFirst)    
...
n = str(input('Enter a word: ')).upper()
sum = 0
...
for i in range(len(n)):
    sum += dictionary[n[i]]
print('You got {} points'.format(sum))
